...
class SceneManager {
  ...
  checkPlanetIntersection(x, y) {
    ...
    const intersects = this.raycaster.intersectObjects(this.planetObjects, true);
    
    if (intersects.length > 0) {
      // Find the parent planet object
      let planetId = null;
      for (const planet of this.planets) {
        if (planet.planetGroup.children.includes(intersects[0].object) ||
            planet.planetGroup === intersects[0].object) {
          planetId = planet.id;
          break;
        }
      }
  ...
  animate() {
    ...
    if (this.starField) {
      this.starField.update(delta, elapsedTime);
    }
